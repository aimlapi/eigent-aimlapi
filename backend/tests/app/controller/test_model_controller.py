...
class TestListProviderModels:
    ...
    @pytest.mark.asyncio
    async def test_attributes_the_model_list_request_to_aimlapi(self):
        client = AsyncMock()
        client.__aenter__.return_value = client
        client.get.return_value = httpx.Response(200, json={"data": []})
        request = ListProviderModelsRequest(
            api_host="https://api.aimlapi.com/v1",
            models_endpoint="/models",
            api_key="secret-key",
        )
        with patch(
    ...
    @pytest.mark.asyncio
    async def test_other_providers_get_no_aimlapi_attribution(self):
        client = AsyncMock()
        client.__aenter__.return_value = client
        client.get.return_value = httpx.Response(200, json={"data": []})
        request = ListProviderModelsRequest(
            api_host="https://api.aimlapi.com.evil.test/v1",
            models_endpoint="/models",
            api_key="secret-key",
        )
        with patch(
            ...
            await list_provider_models(request)
        headers = client.get.await_args.kwargs["headers"]
        assert not [k for k in headers if k.lower().startswith("x-aimlapi-")]
